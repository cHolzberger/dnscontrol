...
coredns -
  Generate zonefiles suitiable for coredns.
...
func initcoredns(config map[string]string, providermeta json.RawMessage) (providers.DNSServiceProvider, error) {
...
// coredns is the provider handle for the coredns driver.
type coredns struct {
...
func panicInvalid(err error) {
	if err != nil {
		panic(errors.Wrap(err, "unparsable record received from coredns"))
	}
}
...
func (c *coredns) GetDomainCorrections(dc *models.DomainConfig) ([]*models.Correction, error) {
	dc.Punycode()
	...
	if _, err := os.Stat(c.directory); os.IsNotExist(err) {
		fmt.Printf("\nWARNING: coredns directory %q does not exist!\n", c.directory)
	}
